<template>
  <div class="summary-wrapper">
    <div class="model-items">
      ...
      <ROIText>
        <template v-slot:header>
          <h3>What is GHI, DHI, and DNI?</h3>
        </template>
        <p> GHI, DHI, and DNI are solar irradiance components. GHI is global horizontal irradiance and is equivalent to the global solar irradiance on a horizontal surface.
        </p>
        <div
            v-katex="'\\small Global\\ Horizontal\\ Irradiance\\ = \\large DHI \\small + \\large DNI \\small * \\cos (Panel Angle)'"></div>   
            <p> 
            DHI is diffuse horizontal irradiance and is the light that reflects/diffuses off of clouds, dust, the ground and other surfaces before hitting the panel. 
            </p>
            <div
            v-katex="'\\small Diffuse\\ Horizontal\\ Irradiance = \\large GHI \\small -  \\large DNI\\ * \\cos (PanelAngle)'"></div>
            <p>
             The equivalent diffuse radiation that hits an angled panel is:
            </p>
            <div
            v-katex="'\\small Panel\\ Angled\\ Diffuse\\ Irradiance = \\large DHI \\small * \\frac{180 - Panel Angle}{180}'"></div>
          <p>
          DNI is direct normal irradiance and is the light that directly hits the panel, perpendicular to its angle. In our model, it is estimated from GHI by using the DIRINT model from <a href="https://pvlib-python.readthedocs.io/en/latest/reference/generated/pvlib.irradiance.dirint.html?highlight=DIRINT#pvlib.irradiance.dirint" target="_blank" rel="noopener noreferrer">pvlib</a> which performs complex time derivative math estimations to get a value (more information on this can be found <a href="https://www.semanticscholar.org/paper/Dynamic-global-to-direct-irradiance-conversion-Ineichen-Perez/0da5f4e6bdb0f42eb10d45607cce1df13e08961a" target="_blank" rel="noopener noreferrer">here</a>).
          To get the value of DNI on a tilted panel, we must use the complex equation below from <a href="https://www.pveducation.org/pvcdrom/properties-of-sunlight/making-use-of-tmy-data" target="_blank" rel="noopener noreferrer">pveducation</a>. The Solar Declination Angle and HRA (Solar Hour Angle) will be explained in the Solar Time section on this page.
          </p>
          <div
            v-katex="'\\small Panel\\ Direct\\ Normal\\ Irradiance = \\large DNI \\small (\\sin (δ) \\sin (φ) \\cos (β)\\ -\\ \\sin (δ) \\cos (φ) \\sin (β) \\cos (ψ)\\ + \\cos (δ) \\cos (φ) \\cos (β) \\cos (HRA)\\ + \\cos (δ) \\sin (φ) \\sin (β) \\cos (ψ) \\cos (HRA)\\ + \\cos (δ) \\sin (ψ) \\sin (HRA) \\sin (β)) \\newline \\space \\newline  where\\ \\newline \\space \\newline δ\\ is\\ the\\ Declination\\ Angle\\ \\newline  φ\\ is\\ the\\ Latitude\\ \\newline  β\\ is\\ the\\ panel\\ tilt\\ \\newline  ψ\\ is\\ the\\ panel\\ azimuth\\ angle\\ (panel\\ direction/orientation\\ angle)\\ \\newline HRA\\ is\\ solar\\ hour\\ angle\\ '"></div>
        <template v-slot:footer> </template>
      </ROIText>
    </div>
    </div>
